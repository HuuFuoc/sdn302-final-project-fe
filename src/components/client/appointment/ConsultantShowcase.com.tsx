...
const ConsultantShowcase: React.FC = () => {
    ...
    <div className="consultant-showcase py-16 bg-gray-50">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-[#20558A] mb-4">Đội ngũ giảng viên</h2>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Tìm hiểu thông tin giảng viên để chọn người phù hợp với mục tiêu học mỹ thuật của bé.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8 justify-between items-center">
          ...
          <Button icon={<ReloadOutlined />} onClick={fetchConsultants} loading={loading} size="large">
            Làm mới
          </Button>
        </div>
        ...
        {loading ? (
          ...
          <Row gutter={[24, 24]}>
            {filteredConsultants.map((consultant) => (
              <Col xs={24} sm={12} lg={8} xl={6} key={consultant.id}>
                <ConsultantCard consultant={consultant} className="h-full" />
              </Col>
            ))}
          </Row>
        ) : (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-gray-600 mb-2">Không tìm thấy giảng viên</h3>
            ...
            {searchTerm && (
              <Button type="link" onClick={() => setSearchTerm("")} className="mt-2">
                Xóa bộ lọc
              </Button>
            )}
          </div>
